...
contract IndexFactoryBalancerV8 is
    Initializable,
    OwnableUpgradeable,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable
{
        ...
        onlyOwnerOrOperator
    {
        ...
        if (bondDeficit) {
            ...
            issuanceRiskAsset(bal, tokenInPath, tokenInFees);
        ...
        returns (bool sold, uint256 qty18Sold)
    {
        ...
        // console.log("shareDiff18: ", shareDiff18);

        uint256 shareDiff18;

        for (uint256 i; i < factoryStorage.functionsOracle().totalCurrentList(); ++i) {
            address tokenAddr = factoryStorage.functionsOracle().currentList(i);
            uint256 current = factoryStorage.functionsOracle().tokenCurrentMarketShare(tokenAddr);
            uint256 oracle = factoryStorage.functionsOracle().tokenOracleMarketShare(tokenAddr);

            if (functionsOracle.tokenAssetType(tokenAddr) == 0) {
                if (current > oracle) {
                    shareDiff18 = (oracle / current) * 100e18;
                    qty18Sold = _sellBond(nonce, tokenAddr, shareDiff18, ctx);
                }
            } else {
                if (current > oracle) {
                    shareDiff18 = (oracle / current) * 100e18;
                    qty18Sold = _redeemRiskAsset(nonce, tokenAddr, shareDiff18, ctx);
                }
            }
        }
    ...
    //     factoryStorage.usdc().approve(factoryStorage.riskAssetFactoryAddress(), usdcAmount + usdcFee);
        ...
        onlyOwnerOrOperator
    {
        ...
        IRiskAssetFactory(factoryStorage.riskAssetFactoryAddress()).issuanceIndexTokens{value: msg.value}(
            address(factoryStorage.usdc()), _tokenInPath, _tokenInFees, usdcAmount - usdcFee
        );
    ...
    function redemptionRiskAsset(
        ...
        if (msg.value < expected) revert WrongETHAmount();
        factoryStorage.usdc().approve(factoryStorage.riskAssetFactoryAddress(), amountIn);
