...
import "@chainlink/contracts/src/v0.8/operatorforwarder/interfaces/ChainlinkRequestInterface.sol";
...
contract MockApiOracle is ChainlinkRequestInterface, LinkTokenReceiver {
    ...
    function fulfillOracleFundingRateRequest(
        bytes32 _requestId,
        address[] memory _tokens,
        uint256[] memory _marketCapShares
    ...
    function uintToBytes32(uint256 myUint) public pure returns (bytes32 myBytes32) {
    ...
    function convertUintToBytes32(uint256[] memory uintArray) public pure returns (bytes32[] memory) {
        uint256 length = uintArray.length;
        ...
        for (uint256 i = 0; i < numChunks; i++) {
            uint256 startIndex = i * 32;
            uint256 endIndex = startIndex + 32;
            if (endIndex > length) {
                endIndex = length;
            }
            uint256[] memory chunk = new uint256[](endIndex - startIndex);
    ...
    function bytes32ArrayFromUintArray(uint256[] memory uintArray) private pure returns (bytes32) {
